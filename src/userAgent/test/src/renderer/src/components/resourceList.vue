<template>
    <div class="resource-list">
        ...
        <div class="resource-cards-wrapper">
            ...
            <!-- 空状态 -->
            <div v-if="props.resources.length === 0" class="empty">
                暂无资源数据
            </div>
...
import ResourceCategory from '@/components/resourceCategory/index.vue'
...
// 规范化 type：从后端返回的大写/API格式转为 CSS 类名对应的格式
const normalizeType = (type) => {
    if (!type) return 'api'
    const t = type.toString().toLowerCase().trim()
    if (t === 'web') return 'web_page'
    return t
}
...
const categories = computed(() => {
    ...
    console.log('[resourceList] props.resources:', items)
    items.forEach(item => {
        const normType = normalizeType(item.type)
        if (!typeMap[normType]) {
            typeMap[normType] = []
        }
        typeMap[normType].push(item)
    })
    const result = Object.entries(typeMap).map(([type, items]) => ({
        type,
        title: type === 'web_page' ? '网页应用' : type === 'api' ? 'API 接口' : type === 'static' ? '静态资源' : type,
        items
    }))
    console.log('[resourceList] categories:', result)
    return result
})
...
/* 滚动条美化 - WebKit 内核浏览器 (Chrome, Safari, Edge) */
.resource-cards-wrapper::-webkit-scrollbar {
    width: 6px;
}

.resource-cards-wrapper::-webkit-scrollbar-track {
    background: transparent;
}

.resource-cards-wrapper::-webkit-scrollbar-thumb {
    background: #c1c1c1;
    border-radius: 3px;
}

.resource-cards-wrapper::-webkit-scrollbar-thumb:hover {
    background: #a1a1a1;
}
